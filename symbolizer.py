...
# build symbol table for class score


# build symbol table for each function by parsing all varDecs in subroutine body


# symbol table structure:
# symbol_tables = [class_symbol, func_smybol, ...]
# class/func_symbol: [var1, var2, ..]
# var1: dict with keys 'name',' type', 'kind', 'num'
...
class Parser():
	...
	def createWhileStatement(self, statement):
		...
		# add expression
		while_exp = etree.SubElement(statement, "expression")
		self.createExpression(while_exp)
		...
		# add statements
		while_states = etree.SubElement(statement, "statements")
		self.createStatements(while_states)
	...
	def createTerm(self, expression):
		...
		if term_token.text.strip() == '-' or term_token.text.strip() == '~':
			# add unary operator
			unary_op = etree.SubElement(expression, "symbol")
			unary_op.text = term_token.text

			# create term
			unary_exp = etree.SubElement(expression, "term")
			self.createTerm(unary_exp)
		...
		if term_token.text.strip() == '(':
			term = etree.SubElement(expression, "symbol")
			term.text = term_token.text.strip()

			term_exp = etree.SubElement(expression, "expression")
			self.createExpression(term_exp)

			end_exp = etree.SubElement(expression, "symbol")
			end_exp.text = self.next_token().text.strip()
	...
	def createExpressionList(self, expList):
		while True:
			...
			if (self.next_token().text.strip() in [';']):
				self.go_back_one()
				self.go_back_one()
				return
			else:
	...
	def createClassVarDec(self):
		...
		while True:
			...
			if (terminate.text.strip() == ','):
				...
				split_var.text = ','
				while True:
					next_var = etree.SubElement(classVarDec, "identifier")
					next_var.text = self.next_token().text.strip()

					if self.next_token().text.strip() == ';':
						self.go_back_one()
						end_var = etree.SubElement(classVarDec, "symbol")
						end_var.text = self.next_token().text.strip()
						break

					self.go_back_one()
					com = etree.SubElement(classVarDec, "symbol")
					com.text = self.next_token().text.strip()
...
class Symbolizer:
	def __init__(self, code_xml):
		self.__program = code_xml
		
		self.__symbol_tables = []

	def symbolize(self):
		# get class dec
		self.symbolize_class_dec()

		subroutineDecs = self.__program.findall('subroutineDec')
		for subroutineDec in subroutineDecs:
			func_name = subroutineDec[2].text.strip()
			par_list = self.symbolize_par_list(subroutineDec)
			subroutineBody = subroutineDec.find('subroutineBody')
			var_list = self.symbolize_loc_vars(subroutineBody)
			subroutine_table = par_list + var_list

			subroutine_symbols = {}
			subroutine_symbols[SCOPE_KEY] = func_name
			subroutine_symbols[SYM_KEY] = subroutine_table

			self.__symbol_tables.append(subroutine_symbols)

		return


	def get_symbol_table(self):
		return self.__symbol_tables
	...
	def symbolize_class_dec(self):
		classVarDecs = self.__program.findall('classVarDec')
		class_vars = []
		num_vars = 0
		for classVarDec in classVarDecs:
			var_new = {}
			values = []
			for var_dec_words in classVarDec.getchildren():
				values.append(var_dec_words.text.strip())

			# assign var keys
			var_new[KIND_KEY] = values[0]
			var_new[TYPE_KEY] = values[1]
			var_new[NAME_KEY] = values[2]
			var_new[NUM_KEY] = num_vars
			
			class_vars.append(var_new)
			num_vars += 1
			...
			if len(values) > 4:
				...
				while not done:
					var_new_2 = {}
					var_new_2[KIND_KEY] = values[0]
					var_new_2[TYPE_KEY] = values[1]
					var_new_2[NAME_KEY] = values[name_idx]
					var_new_2[NUM_KEY] = num_vars
					num_vars += 1
					class_vars.append(var_new_2)
					if (name_idx + 2) > (len(values) - 1):
						done = True
					else:
						name_idx += 2
		class_symbols = {}
		class_symbols[SCOPE_KEY] = 'class'
		class_symbols[SYM_KEY] = class_vars
		self.__symbol_tables.append(class_symbols)

		return

	def symbolize_sub_dec(self, subroutineDec):
		subroutine_vars = []
		subroutine_children = subroutineDec.getchildren()

		par_list = self.symbolize_par_list(subroutineDec)
		subroutineBody = subroutineDec.find('subroutineBody')
		local_vars = self.symbolize_loc_vars(subroutineBody)
		subroutine_vars = par_list + local_vars
		subroutine_symbols = {}
		subroutine_symbols[SCOPE_KEY] = subroutine_children[2].text.strip()
		subroutine_symbols[SYM_KEY] = subroutine_vars
		self.__symbol_tables.append(subroutine_symbols)

	def symbolize_par_list(self, subroutineDec):
		num_args = 0
		par_list = subroutineDec.find('parameterList')
		par_vars = []
		values_buffer = []
		for par in par_list.getchildren():

			# if we see ','
			if par.text.strip() == ',':
				var_new = {}
				var_new[NAME_KEY] = values_buffer[1]
				var_new[TYPE_KEY] = values_buffer[0]
				var_new[KIND_KEY] = 'argument'
				var_new[NUM_KEY] = num_args
				par_vars.append(var_new)
				num_args += 1

				#clear buffer
				values_buffer.clear()

			# we are reading in a variable
			else:
				values_buffer.append(par.text.strip())

		# take care of remaining buffer			
		if len(values_buffer) > 0:
			var_new = {}
			var_new[NAME_KEY] = values_buffer[1]
			var_new[TYPE_KEY] = values_buffer[0]
			var_new[KIND_KEY] = 'argument'
	...
	def symbolize_loc_vars(self, subroutineBody):

		all_var_decs = subroutineBody.findall('varDec')

		local_vars = []
		num_local = 0
		for var_dec in all_var_decs:
			values = []
			for term in var_dec.getchildren():
				values.append(term.text.strip())

			var_new = {}
			var_new[NAME_KEY] = values[2]
			var_new[TYPE_KEY] = values[1]
			var_new[KIND_KEY] = 'local'
			var_new[NUM_KEY] = num_local

			local_vars.append(var_new)
			num_local += 1
			...
			if len(values) > 4:
				...
				while not done:
					var_new_2 = {}
					var_new_2[KIND_KEY] = 'local'
					var_new_2[TYPE_KEY] = values[1]
					var_new_2[NAME_KEY] = values[name_idx]
					var_new_2[NUM_KEY] = num_local
					num_local += 1
					local_vars.append(var_new_2)
					if (name_idx + 2) > (len(values) - 1):
						done = True
					else:
						name_idx += 2
...
if __name__ == '__main__':
	...
	symbolizer = Symbolizer(parse_tree)

	symbolizer.symbolize()

	f_symbols = symbolizer.get_symbol_table()
